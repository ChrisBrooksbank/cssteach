...
- [x] Build shared `LiveEditor` component — split-pane CSS input + sandboxed HTML preview, used across all 4 topic areas
- [x] Build topic page shell — `/[topic]` route with Tutorials / Reference / Challenges tab navigation, consistent header
...
- [x] Selectors challenges — Select the right elements, specificity battles, debug broken stylesheets
...
- [x] Typography tutorials — Web fonts, color formats (hex, rgb, hsl, oklch), and CSS custom properties for theming
- [x] Typography reference — Property reference table and color format comparison chart with live swatches
- [x] Typography challenges — Match typography styles, build HSL color palettes, and theme switcher with custom properties
...
- [x] Animations challenges — Hover effects, loading spinners, page transitions, and interactive button state transitions

### Phase 6: Testing

- [x] Unit tests for `LiveEditor` and tab navigation components
- [x] E2E tests covering each topic module's happy path (load, interact, check output)
- [x] Update Playwright visual regression baselines for all 4 topic pages
